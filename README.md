...
2. Create an S3 bucket called test-enem-bucket (give a suitable name for your use case)
3. Create an IAM User called test-enem and grant it admin permissions and save the access keys.
...
7. Create a new Lambda function from scratch, name it process-enem-pdf, choose Python 3.9 runtime. lambda will automatically create an IAM Role. Make sure this role has the read and write permissions in the test-enem-bucket S3 bucket. Increase the execution time to about 4 mins to the lambda.
...
12. Create a glue Crawler to create a catalog of the dataset. Name it: test-enem-crawler and make sure to select the bucket up to the content folder. Make sure an IAM role is created and also create a database with the name: enem_pdf_project
